---
title:  "5 Efficient Ways to Collect Show Commands Output From Network Devices"
summary: Simple but not easy task
authors:
    - Denis Mulyalin
date: 2022-08-21
---
...
Can copy files from proxy minion or use `nr.file` function to retrieve output
at any time. For example to read file content for ceos1 host only:
...
Its certainly going to take a lot of typing. More efficient way is to save commands
into a text file on salt-master machine and request proxy minions to source commands
to collect form that file.
...
What if command need to contain per host details like source IP address as in ping
or traceroute commands, solution is to use Jinja2 templates:
...
But what if need to collect different commands output from different device types, Jinja2
conditionals certainly can handle it, here is `/etc/salt/cli/conditional_commands.txt`
file content:
...
Another use case - what if I need to run commands on a per host basis, do I need to use
Jinja2 conditionals as well - yes, can rely on Jinja2 conditional or can supply per-host
files with commands. For example, this is the content of `/etc/salt/cli/commands_ceos1.txt`
and `/etc/salt/cli/commands_ceos2.txt` files respectively:
...
Troubleshooting network problems often can benefit from collecting show commands output
...
when problem is reproducible but only lasts for seconds
...
`table=brief` instructs Salt-Nornir to return nicely formatted text table to make it
easier to read through the output.

Another feature is capability to stop collecting show commands after certain pattern
found in the output. Made up example - you want to collect "show ip route" output in a
loop but stop if certain route becomes visible in the routing table.
...
Salt-Nornir makes it extremely easy to switch between connection methods making sure
you have flexibility to choose what works best for you.
...
Since Netmiko is a default connection method, no need to specify plugin to use. However,
this is how different plugins can be invoked:
...
Besides using different plugins, nr.cli also support
[promptless (ps) mechanism](https://nornir-salt.readthedocs.io/en/latest/Tasks/netmiko_send_command_ps.html)
to collect output from devices. This mechanism based on Netmiko library but adds several enhancements
to make process of collecting big amount of output from network devices more reliable.
...
- does not timeout on big chunks of data as long as device keeps transmitting data thanks to
  using read and absolute timeouts
